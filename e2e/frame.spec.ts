...
test('Counter should work in a frame', async ({ page }) => {
  ...
  await page.goto('http://counter.info/frame.html', {
    referer: 'http://counter.info/frame-previous.html',
  });
  ...
  requests.forEach((params) => {
    ...
    expect(params.get('url')).toBe('http://counter.info/frame.html');
    expect(params.get('ref')).toBe('http://counter.info/frame-previous.html');
  });
});
